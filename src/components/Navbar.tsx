...
export function Navbar({
  ...
  const crudeQuote = getQuote("CRUDEOIL") || getQuote("CRUDEOIL FUT") || getQuote("Crudeoil");
      ...
      <div className="max-w-[1700px] mx-auto px-3 sm:px-4 py-2.5 space-y-2.5 lg:space-y-0 lg:flex lg:items-center lg:justify-between lg:gap-4">
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              className={`px-2.5 py-1.5 sm:px-3 sm:py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-all border cursor-pointer ${
                powerOn
        ...
        <div className="lg:hidden flex items-center justify-between gap-2 overflow-x-auto no-scrollbar py-0.5">
          <nav className="flex items-center gap-1 bg-slate-900/90 p-1 rounded-xl border border-slate-800 w-full justify-around sm:justify-start">
            ...
            <button
              onClick={() => onSelectScreen("orders")}
              className={`px-2.5 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer relative shrink-0 ${
                activeScreen === "orders"
                  ? "bg-teal-500 text-slate-950 shadow-md shadow-teal-500/20"
                  : "text-slate-400 hover:text-slate-100"
              }`}
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            {crudeQuote ? (
              <div className="flex items-center gap-1">
                ...
                <span
                  className={`text-[10px] font-bold font-mono ${
                    crudeQuote.change >= 0 ? "text-emerald-400" : "text-rose-400"
                  }`}
                >
                  {crudeQuote.change >= 0 ? "+" : ""}
                  {crudeQuote.change.toFixed(2)} ({crudeQuote.changePct.toFixed(2)}%)
                </span>
              </div>
            ) : (
              <span className="text-[10px] text-slate-600 animate-pulse font-mono">
